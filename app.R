...
ui <- fluidPage(
  titlePanel("Takhle by to mohlo být"),
  column(3,
         sliderInput(inputId = "week0",
              label = "Hodnota R pro tento týden",
              value = 1.5, min = 0, max = 10, step = 0.1, playButton),
  ...
  sliderInput(inputId = "week4",
              ...
              value = 1.5, min = 0, max = 10, step = 0.1)),
 column(3, dateInput(inputId = "date_model_start", label = "choose date for model starts", 
            value = today(tzone = "GMT") - 1),
  dateInput(inputId = "date_model_end", label = "choose date for model end (latest end of year)", 
...
server <- function(input, output, session) {
  ...
  output$chart <- renderPlot({
    ...
    CZ_expected_deaths_timeline_projected %>% 
      ...
      labs(y = "Deaths (7 day rolling average)", x = "", title = "Long-term scenarios of expected daily deaths", 
           subtitle = "Based on past data and 4 scenarios of growth in cases")
  })
